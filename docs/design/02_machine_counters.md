...
为避免后续 interrupt 决策与流水动作互相组合依赖，先定义一个不依赖
`mem_wb_action` 的提交候选条件，再定义真正一次性提交事件：
...
`mem_commit_candidate` 表示 MEM 指令已完成所有可能产生异常的工作，可以用于
计算 post-commit 事件，但它本身不表示流水已经接收。`commit_valid` 表示该候选
本拍确实被 MEM/WB 接收，此后一定会在 WB 正常退休。外部 retire 事件仍保持现有
时序，只是比内部 commit 晚一个流水级。

`mem_wb_candidate` 的形成必须独立于 `mem_wb_action`、`interrupt_take` 和
`commit_valid`；它是仲裁的输入，不是仲裁结果。
...
- post-commit interrupt 只能使用 `mem_commit_candidate` 做决策，不能用
  `commit_valid` 反向驱动 `mem_wb_action`；
- post-commit interrupt 成立时，必须保证同拍 `commit_valid=1`。
...
`mem_wb_q.valid` 或 `retire_valid` 近似代替。`mem_commit_candidate` 只用于提交前的仲裁，
不能直接驱动 `minstret` 或架构副作用。
