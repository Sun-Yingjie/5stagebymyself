...
- 统一 WB 退休接口；
- Zicsr 流水契约、无状态 CSR 运算/语义译码叶子、独立 CSR/trap 状态所有者，以及尚不可达的被动流水字段与 late-result 控制路径；
...
- Zicsr 已完成架构契约、无状态 CSR 运算/语义译码叶子、被动流水字段、通用 late-result 冒险控制和独立 CSR/trap 状态所有者；owner 尚未接入 core，主 decoder 仍保持 CSR illegal，因此 CSR 指令仍不可执行；
